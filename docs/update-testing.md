...
| V5 new DLC depot (2026-09-17) | simulate a DLC that just appeared: drop the key line of Brotato's Abyssal Terrors depot (2868390) from `keys.txt`, clear `~/.cache/lumadeck/hubcap_attempts.json` so the daily budget does not skip the app, restart Decky (the 30-min pass runs at start) | **pass, LumaDeck half** — `zip for 1942280 from 'Morrenus' (32298 bytes)`, `update check 1942280: reinstalled from zip: new depots [2868390], build 23429717`, key line back. The Steam half did not run because Steam never uninstalled the depot when its licence vanished (`.acf` still listed `2868390` with its manifest), so there was nothing to download; that half is the 2026-09-13 measurement (RESEARCH §19.3: key added → "Update Optional", 55 MB) plus T1/T2. Note for the record: keys for a new depot come **only** from the zip cascade (Hubcap with the user's key, Ryu with a cookie) — without those, `no source returned one` daily, forever, and the DLC stays invisible |
