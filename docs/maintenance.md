...
| `PKG0_FINDER: node idx N has key 0 but the PackageInfo at … reports PackageId=M` | The finder reached package 0 through the tree but the object disagrees about who it is: a torn/stale read, or the class offsets no longer describe `CPackageInfoCache`. It refuses to write and retries on the next poll — if it never stops, the offsets moved | **C** Finder anchors |
