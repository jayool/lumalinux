...
**Nada que actualizar. `LumaCore/` no se ha tocado.**

Un solo commit nuevo desde el cutoff de este doc: `a26a359` "Release SteaMidra
v6.6.5" (2026-08-16). SFF aplasta cada release en un commit único, así que son
43.213 inserciones en 37 ficheros — pero `git show a26a359 --stat -- LumaCore` no
devuelve nada: todo está en el launcher Python (`sff/`) y la web UI. El análisis
de este documento sigue vigente sobre la capa C++ que analiza.

Dos cosas del launcher que sí rocé, ambas descartadas al contrastarlas con
nuestro código, anotadas sólo para no volver a perseguirlas:

- `prewarm_pattern_cache_if_missing` precalienta la caché de patrones desde
  Python. No nos aplica: `main.cpp:428` ya instala los hooks en un
  `std::thread(...).detach()`, así que la descarga del feed nunca estuvo en el
  camino del cargador.
- Fallback de versión leyendo `status.json`. LumaDeck ya lo hace mejor —
  `paths.py:756-769` lo usa como fuente primaria, no como plan B.

*(Contexto de ecosistema, fuera del alcance de este doc: SteaMidra empaqueta una
base de 368.230 claves de depot alimentada por sus usuarios — `provider.py`
